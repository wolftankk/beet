registerMenu("settings", "cardAdmin", "产品管理",
    [
	{
	    text: "费用设置",
	    handler: function(){
		var item = Beet.cache.menus["cards.UpdateCharge"];
		if (!item){
		    Beet.workspace.addPanel("cards.UpdateCharge", "费用设置", {
			items: [
			    Ext.create("Beet.apps.cards.UpdateCharge")
			]
		    })
		}else{
...
function createChargeCategoryTree(){
  ...
  me.createTreeList = function(){
    ...
    me.storeProxy = store = Ext.create("Beet.apps.cards.ChargesCatgoryTreeStore");
...
Ext.define("Beet.apps.cards.UpdateCharge", {
  ...
  initComponent: function(){
    var me = this;
    me.callParent();
    me.add(Ext.create("Beet.apps.cards.ChargeList"));
    me.doLayout();
  }
});

Ext.define("Beet.apps.cards.ChargeList", {
  extend: "Ext.panel.Panel",
  autoHeight: true,
  autoScroll: true,
  height: Beet.constants.VIEWPORT_HEIGHT - 5,
  ...
  buildStoreAndModel: function(metaData){
    ...
    if (!Beet.apps.cards.ChargeTypeModel){
      Ext.define("Beet.apps.cards.ChargeTypeModel", {
        extend: "Ext.data.Model",
        fields: fields
      });
    }

    if (!Beet.apps.cards.ChargeTypeStore){
      Ext.define("Beet.apps.cards.ChargeTypeStore", {
        extend: "Ext.data.Store",
        model: Beet.apps.cards.ChargeTypeModel,
        autoLoad: true,
  ...
  createGrid: function(){
    ...
    Ext.apply(this, {
      storeProxy: Ext.create("Beet.apps.cards.ChargeTypeStore")
    });
      ...
      tbar: [
        ...
        "-",
        {
          ...
          handler: function(){
            ...
            win.add(Ext.create("Beet.apps.cards.AddCharge", {
              _editType: "add"  
            }))
  ...
  editChargeType: function(parentMenu){
    ...
    var f = Ext.create("Beet.apps.cards.AddCharge", config);
