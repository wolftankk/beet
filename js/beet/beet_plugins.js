...
Beet.plugins.customerDropDown = function(){
    ...
    var buildCustomerStore = function(){
        ...
        customerServer.GetCustomerPageData(0, 1, "", {
            success: function(data){
                ...
                me.customerStore.setProxy(updateCustomerProxy(""));
            },
            failure: function(error){
                Ext.Error.raise(error)
            }
    ...
    this.getCustomerStore = function(){
	return me.customerStore
    }
	...
	if (f.picker == undefined){
		...
		itemClick: function(v, record, item, index, e, opts){
		    that.onSelectCustomer(record);
		    that.selectedCustomer = true;
		    f.picker.hide();
		},
		refresh: function(){
		    setTimeout(function(){
	...
	me.customerStore.setProxy(updateCustomerProxy("CTName LIKE '%"+newValue+"%'"));
